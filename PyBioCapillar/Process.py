...
if len(DataProc)==0:
    ...
    Proj1=ZProj.sum(axis=1)
    Proj0=ZProj.sum(axis=0)
    Proj0_medP=len(Proj0)/2
    Proj1_medP=len(Proj1)/2
    ##Axis 0 find two maximas
    ####left side
    LeftP=Proj0[0:Proj0_medP].argmax()
    ####right side
    RightP=Proj0[Proj0_medP:len(Proj0)].argmax()+Proj0_medP
    Ctrd0=(RightP+LeftP)/2
    ##Axis 1 find two maximas
    ####left side
    LeftP=Proj1[0:Proj1_medP].argmax()
    ####right side
    RightP=Proj1[Proj1_medP:len(Proj1)].argmax()+Proj1_medP
    Ctrd1=(RightP+LeftP)/2
    Rh=len(Proj0)/2-Ctrd0
    Rv=len(Proj1)/2-Ctrd1
    ###print(Rh,Rv)
    ###patata
